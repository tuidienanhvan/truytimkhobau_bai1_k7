...
// Component giờ tự kết nối store, không cần props
export const MissionLog: React.FC = React.memo(() => {
    const logs = useGameStore(s => s.logs); // Atomic selection
    ...
    }, [logs.length]); // Chỉ scroll khi số lượng log thay đổi

    // MathJax Rendering for Logs using Utility
    useEffect(() => {
