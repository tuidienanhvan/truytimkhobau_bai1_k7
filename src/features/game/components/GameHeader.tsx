...
import { Rocket, Trophy, Maximize, Minimize, User, Activity, ShoppingBag, Coins } from 'lucide-react';
...
export const GameHeader: React.FC<GameHeaderProps> = React.memo(({ 
  ...
  const showShopButton = gameState === GameState.WELCOME;
  ...
  return (
    <div className="h-24 pl-8 pr-0 flex items-center bg-space-950 border-b-[3px] border-neon-blue/60 shrink-0 select-none relative z-50 font-tech shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-5 shrink-0 z-10">
        <div className="p-3 bg-neon-blue rounded-lg shadow-[0_0_20px_rgba(0,240,255,0.4)] relative group overflow-hidden">
             <div className="absolute inset-0 bg-white/30 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
             <Rocket size={40} className="text-black relative z-10" />
        </div>
        <div className="flex flex-col justify-center">
          <span className="text-sm font-bold tracking-[0.3em] text-slate-500 uppercase leading-none mb-1.5">PiStudy</span>
      ...
      <div className="flex-1 flex items-center justify-end mr-8 gap-8 z-10">
        ...
        {/* ADDED: SHOP BUTTON (Only in Welcome) */}
        {showShopButton && (
            <button 
                onClick={handleOpenShop}
                className="flex items-center gap-3 bg-neon-blue/10 border border-neon-blue/50 text-neon-blue px-6 py-2 rounded skew-x-[-10deg] hover:bg-neon-blue hover:text-black hover:shadow-[0_0_20px_rgba(0,240,255,0.5)] transition-all group mr-4"
            >
                <ShoppingBag size={20} className="skew-x-[10deg]" />
                <span className="font-bold tracking-widest skew-x-[10deg]">KHO VŨ KHÍ</span>
            </button>
        )}

        {/* ADDED: COINS DISPLAY (Always visible) */}
        ...
        {/* PILOT IDENTITY (RESTORED ORIGINAL) */}
